...
The **fixed patient-level 7:1:2 partition** currently provided in this
repository is `data/splits/brats2021/`:
...
Exact LUNA16 / VinDr-CXR split manifests are **not included** in this
repository at this time.  `scripts/preprocess.py` accepts
`--split-manifest-root data/splits` and uses a frozen manifest
(`<root>/<dataset>/{train,val,test}_patients.txt`) verbatim when present;
otherwise it generates a patient-level 7:1:2 split from `--split-seed` and
records the split source in `protocol.json`.
